...
- 同步 `js-reverse-mcp-cloak` 能力：记录项目级 CloakBrowser sidecar、`container/runner/bin/js-reverse-mcp-cairn` slot lease/release wrapper、noVNC 项目 UI 状态入口和 `worker.cloak_sidecar` 配置。
...
- 修正文档漂移：`server.yaml` 真实 YAML key 为 `app/database/security/admin/dispatcher/storage/worker/runner/tool_sidecars`；加载后才映射到内部 `server.auth`、`worker_runtime.runner` 等模型路径。
